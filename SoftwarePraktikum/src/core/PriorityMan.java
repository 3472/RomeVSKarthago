...
public class PriorityMan extends KIEasyJ{
	...
	public Move makeMove(City_Graph cityGraph, History h, Move prevMove) {
		 ...
		 * der Gegner, geben wir einen Illegalen move zuck und haben
		 * gewonnen
		 */
		if(h.wasLastMoveIllegal()){
			if(cityGraph.getScore(thisPlayer) > cityGraph.getScore(otherPlayer)){
				return forfitMove();
			}
		}
	...
	private void prepareSurround(){
		...
		while(ids.hasNext()){
			...
			for(City c : neuts){
				City_Graph testGraph = tmpG.gameStateTransition(
	...
	private void setSafeCaptureCitys(City_Graph cg){
		for(VertexSet al : safeAreas){
			...
			for(City c : neigs){
				if(c.getOwner() == Owner.Neutral && possibles.containsKey(c.getID())){
					addPoints(c.getID(), 1);
				}
			}
		}
	...
	private void getPossibleCitys(City_Graph cityGraph, History h){
		...
		for(City c : cityGraph){
			if(c.getOwner() == Owner.Neutral){
				City_Graph tmpGraph = cityGraph.gameStateTransition(
						new Move(name, c.getID()));
				if(tmpGraph.getCity(c.getID()).getOwner() != Owner.Neutral){
					...
					if(neutralSet.getSize() < 3){
						VertexSet neigs = cityGraph.getVertexSetNeighbourhood(neutralSet);
						dontAdd = true;
						for(City bla : neigs){

			...
			for(ArrayList<City> cityList : possibleFields){
				...
				for(int i = 0; i < cityList.size(); i++){
					...
					if(!this.givesOneTurnCaptureArea(testGraph, testGraph.getCity(bestCity.getID()))){
						...
						best.put(bestCity.getID(), best.get(bestCity.getID()) + add);
					}
				}
			}
		}
    		...
    		if(targetCity.getOwner() == Owner.Neutral){
    			...
    			if(testGraph.getScore(PlayerToOwner(this.name)) > currentScore
    				...
    				if(testGraph.getScore(PlayerToOwner(this.name)) > mostPoints){
    					mostPoints = testGraph.getScore(PlayerToOwner(this.name));
    	...
    	while(it.hasNext()){
    		...
    		if(targetCity.getOwner() == Owner.Neutral){
    			...
    			if(neutrals > 1){
    				...
    				best.put(targetCity.getID(), best.get(targetCity.getID()) + add);
    			}
    		}
